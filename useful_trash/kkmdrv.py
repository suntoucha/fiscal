...
def dbg(*a):
    if DEBUG:
        print unicode(string.join(map(lambda x: str(x), a)),
                      'utf8').encode(OS_CP)
...
DEFAULT_ADM_PASSWORD = bufStr(0x1e, 0x0, 0x0, 0x0)

# Пароль кассира по умолчанию = 1
DEFAULT_PASSWORD = bufStr(0x1, 0x0, 0x0, 0x0)
...
class KKM:
    ...
    def Sale(self, count, price, text=u"",
        ...
        if department not in range(0, 17):
    ...
    def returnSale(self, count, price, text=u"",
                   department=1, taxes=[0, 0, 0, 0][:]):
        """Возврат продажи
        ...
        if float2100int(price) < 0 or float2100int(price) > 9999999999:
            raise RuntimeError("Price myst be in range 0..9999999999")
        if department not in range(1, 16 + 1):
            raise RuntimeError("Department myst be in range 1..16")
        if len(text) > 40:
            raise RuntimeError("Text myst be less than 40 chars")
        if len(taxes) != 4:
...
#        time.sleep(0.8)
    ...
    def printString(self, check_ribbon=True, control_ribbon=True, text=u""):
        ...
        while len(t) > 0:
            self._printString(
                check_ribbon=check_ribbon,
                control_ribbon=check_ribbon,
                text=t[:39]
            )
            t = t[39:]
    ...
    def _printString(self, check_ribbon=True, control_ribbon=True, text=u""):
        ...
        if control_ribbon:
            flag = flag | 2
        if len(text) > 40:
            raise RuntimeError("Length of string myst be less or equal 40 ch")
    ...
    def Run(self, check_ribbon=True, control_ribbon=True,
            doc_ribbon=False, row_count=1):
        """Прогон"""
        self.__clearAnswer()
        flag = 0
        if check_ribbon:
...
class ShtrihFRK(ShtrihFRKDummy):
    ...
    def printCheck(self, header, summ, comment, buyer, ctype=0, nds=0):
        ...
        if nds > 0:
            ...
            self.kkm.setTableValue(self.admin_password, 6, 2, 1,
                                   pack('l', nds * 100)[:2])

        self.kkm.openCheck(ctype * 2)
        taxes = [0, 0, 0, 0]
        for l in header.split('\n'):
            self.kkm.printString(text=l)
        if ctype == 0:
            self.kkm.printString(text=u"Принято от: %s" % buyer)
            # if nds>0:
            #     taxes[0]=2
            ...
            # if nds>0:
            #     taxes[3]=2
            self.kkm.printString(text=u"Возвращено: %s" % buyer)
            taxes = [0, 0, 0, 0]
            self.kkm.returnSale(1, summ, text=comment, taxes=taxes)
        if nds > 0:
            taxes[0] = 2
        self.kkm.closeCheck(summ,
                            text=u"------------------",
                            taxes=taxes)
...
#
    sfrk = ShtrihFRK(DEFAULT_PASSWORD,
...
#        kkm.Beep()
#        time.sleep(1)
#        kkm.printString(string=u"Привет всем")
...
#        kkm.printString(check_ribbon=True,control_ribbon=True,string=u"Инна Привет")
